  ...
  */
    ...
    akka {
      ...
      actor {
        creation-timeout = 6000s
        default-dispatcher {
         type = akka.dispatch.PCTDispatcherConfigurator
       }
      }
    }
    """
  ...
  def startActor(actorRef: ActorRef) {
    ...
    f.onComplete {
      case Success(value) =>
        if (!value) {
          AkkaActorState.actorLatch.countDown()
          println(Console.BLUE + "Running onComplete on: " + Thread.currentThread().getName + Console.RESET)
          System.exit(0)
        }
